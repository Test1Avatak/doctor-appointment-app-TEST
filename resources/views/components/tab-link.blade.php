@props([
    'tab',
    'error' => false,
])

<li class="me-2">
    <a href="#"
       @click.prevent="tab = '{{ $tab }}'"
       :class="{
         'text-red-600 border-red-600': @json($error) && tab !== '{{ $tab }}',
         'text-red-600 border-red-600 active': @json($error) && tab === '{{ $tab }}',
         'text-blue-600 border-blue-600': !@json($error) && tab === '{{ $tab }}',
         'border-transparent hover:text-blue-600 hover:border-gray-300': !@json($error) && tab !== '{{ $tab }}',
       }"
       class="inline-flex items-center justify-center p-4 border-b-2 rounded-t-lg group transition-colors duration-200"
       :aria-current="tab === '{{ $tab }}' ? 'page' : null"
    >
        {{ $slot }}

        @if($error)
            <i class="fa-solid fa-circle-exclamation ms-2 animate-pulse"></i>
        @endif
    </a>
</li>
